...
Feature: Feature#216: Cucumber module: User Story #225: Log test failures to Dashboard
  ...
  Scenario: Send error data to Dashboard
    ...
    And I check the errors in the dashboard for this test:
      ...
      | QAT::Formatter::Dashboard | Minitest::Assertion | mdc with test ids tests | MDC with multiple outline tables with ids - all enabled   | @top_level_tag,@mdc_with_ids,@mdc_with_ids_error | And false | 3,still_temp,yet_another | 3              | Caught Minitest::Assertion: Expected false to be truthy. |
      | QAT::Formatter::Dashboard | Minitest::Assertion | mdc with test ids tests | MDC with multiple outline tables with ids - all enabled   | @top_level_tag,@mdc_with_ids,@mdc_with_ids_error | And false | 4,temp,yet_another       | 4              | Caught Minitest::Assertion: Expected false to be truthy. |
      ...
      | QAT::Formatter::Dashboard | Minitest::Assertion | mdc with test ids tests | MDC with multiple outline tables with ids - some disabled | @top_level_tag,@mdc_with_ids,@mdc_with_ids_error | And false | 4,temp,yet_another       | 4              | Caught Minitest::Assertion: Expected false to be truthy. |
